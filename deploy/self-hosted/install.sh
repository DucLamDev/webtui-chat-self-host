...
usage() {
  echo "Usage: $0 --domain chat.example.com --email admin@example.com [--name 'Example Chat'] [--logo-url https://chat.example.com/logo.png] [--registration-mode open|invite_only|closed] [--portal-origin https://download.webtui.vn] [--external-ip 203.0.113.10] [--push-mode none|direct-fcm|relay-client|relay-server] [--firebase-service-account-json /path/adminsdk.json] [--firebase-service-account-base64 BASE64] [--push-relay-url URL] [--push-relay-token TOKEN] [--push-relay-publishers UUID=TOKEN] [--enable-web-push] [--skip-dns-check] [--force]"
}
...
CORS_ALLOWED_ORIGINS=https://$DOMAIN,https://$DOMAIN:8444,$PORTAL_ORIGIN,http://tauri.localhost,https://tauri.localhost,tauri://localhost
...
# Use configure-notifications.sh to opt in to direct FCM, publisher relay,
# relay server, or Web Push after the instance has a stable discovery UUID.
...
if [ -n "$(printf '%s' "$NOTIFICATION_CONFIGURE_ARGS" | tr -d '\n')" ]; then
  echo "Configuring notification delivery from installer options..."
  # shellcheck disable=SC2086
  set -- $NOTIFICATION_CONFIGURE_ARGS --restart
  sh "$SCRIPT_DIR/configure-notifications.sh" "$@"
fi
